...
export function ProjectImage({frontmatter}: ProjectImageProps) {
  const hasImages = frontmatter.images?.length > 0;
  ...
  return (
    <div className="relative">
      <div className="w-full h-72 xl:h-80 rounded-3xl overflow-hidden shadow-2xl bg-gradient-to-br from-neutral-50 to-neutral-100 dark:from-neutral-900/50 dark:to-neutral-800">
        {hasImages ? (
          <Swiper
            ...
            loop={frontmatter.images!.length > 1}
          >
      ...
      {hasImages && frontmatter.images!.length > 1 && (
        <>
          <div className={`img-prev-${frontmatter.index} absolute -left-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all cursor-pointer z-10 flex items-center justify-center hover:shadow-xl`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          </div>
          <div className={`img-pagination-${frontmatter.index} absolute -bottom-8 left-1/2 -translate-x-1/2 flex gap-1`} />
          <div className={`img-next-${frontmatter.index} absolute -right-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all cursor-pointer z-10 flex items-center justify-center hover:shadow-xl`}>
