---
# the default layout is 'page'
layout: about
icon: fas fa-info-circle
order: 5
---

# **김주엽**
- Email: <kjye.ops@gmail.com>
- Github: <https://github.com/juye-ops>

## EDUCATION
---
금오공과대학교  
- B.S, Dept. of Computer Engineering
- Overall: 4.13 / 4.5 (Major: 4.24 / 4.5)

## RESEARCH AREA
---
Computer Vision, Self-supervised Learning, Video Recognition

## PROJECTS
---
#### [GRU 기반 행동 분석 모델을 이용한 어린이 이상 행동 검출 시스템](/posts/GRU_Behavioral_analysis/)
2022.03. ~ 2022.08.  
- 아동학대 방지를 위한 비디오 기반 이상 행동 검출 시스템
- MoblieNetV3을 통해 추출한 특징 벡터를 GRU 에 입력하여 행동 분류
- Detection 모델을 통하여 사람 외 노이즈를 제거함으로써 성능을 향상

#### [개인 맞춤 정보 제공 전자 게시판](/posts/Post_recommendation/)
2021.10. ~ 2022.04.
- 교내 학생들에게 유용한 정보를 개별적으로 제공해주고자 고안된 기사 추천 시스템
- QR 코드를 입력한 학생의 정보를 바탕으로 기사와의 유사성을 판단하여 추천 기사 선별
- 서버를 논리적으로 구성함으로써 통신의 효율성을 향상

#### [2021 근골격계 동영상 데이터 인공지능 모델 알고리즘 개발 해커톤](/posts/Musculoskeletal/)
2021.11. ~ 2021.12.  
- 제공하는 데이터셋(동영상)을 재활치료 종류에 따라 분류
- CNN을 통한 Feature를 LSTM 기반으로 학습하여 추론

#### [문서 객체의 랜덤 배치를 통한 문서 레이아웃 분석기](/posts/Document_layout_analysis/)
2021.09 ~ 2021.12.
- Data Manipulation에 따른 Document Layout Analysis 분야 연구
- PublayNet 문서 데이터셋의 Layout patches 를 무작위로 재배치함으로써 높은 mAP를 달성
- 배경이 없는 Object patches 가 갖는 의미를 실험적으로 도출

#### [객체 탐지 데이터 및 학습 관리를 위한 웹 기반 딥러닝 지원 시스템](/posts/Web-based_DL_platform/)
2021.01 ~ 2021.08.
- 웹 기반 딥러닝 지원 플랫폼 서비스
- Object detection 학습에 대한 End-to-End 서비스를 Web-UI 로 제공

#### [이미지 분류와 객체 검출 기반의 컴퓨터 부품 조립 위치 확인 시스템](/posts/Detecting_assembling_position/)
2020.09. ~ 2020.12.
- 식별 된 컴퓨터 부품에 대한 메인보드 상의 결합 위치를 Detection으로 검출

#### [딥러닝 기반 객체 인식을 활용한 퍼스널 모빌리티 안전 보조 시스템 개발](/posts/Personal_mobility/)
2020.07. ~ 2020.10.
- 퍼스널 모빌리티 운전자의 안전을 도모하기 위한 시스템
- 특정 물체와의 거리에 따른 경고 신호 발생


## AWARDS
---
- 한국정보기술학회 2022년도 하계종합학술대회 및 대학생논문경진대회 - **금상 수상**
- 2021 근골격계 동영상 데이터 인공지능 모델 알고리즘 개발 해커톤 - **2위 우수상 수상**

## TEACHING EXPERIENCES
---
#### 신입생 멘토
2021.03. ~ 2022.02.

#### 전공 특화 멘토 - 알고리즘
2021.09. ~ 2021.12.

#### 기초학력 향상 멘토 - Python
2021.03. ~ 2021.06.


## EXTRACURRICULAR ACTIVITIES
---
#### 부스트캠프 AI Tech 4기 [NAVER Connect Foundation]
2022.09. ~ 2023.02.  

#### AI Grand Challange 2020
2020.07.
- 비정형 폐기물 이미지를 생활 폐기물 배출 기준에 따라 구분

#### 포스트 코로나 AI 챌린지 [KT]
2020.04.
- AI를 활용하여 감염병으로부터 국민안전에 기여할 수 있는 서비스 제안