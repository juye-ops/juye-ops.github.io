...
> 성능 좋게 나왔지만, SOTA Document layout analysis 분야에서도 비슷한 성능이 1위를 하고있다.
> 실험을 잘못한 것 같다. 너무 성급하게 한게 아닐까?
...
|프로젝트 요약도|RPLC(좌) 이미지와 RPSLC(우) 이미지|
|:-:|:-:|
|<img src="/static/img/Projects/DLA_blueprint.png">|<img src="/static/img/Projects/DLA_RPLCvsRPSLC.png">|
...
- 카테고리 재구성
  ...
  - 최종 데이터셋
    - PublayNet 카테고리: Figure, Text, Title, **List**, Table
    - Custom Dataset 카테고리: Figure, **Graph**, **Subtitle**, Table, Text, Title
...
---
## 배치 알고리즘
인공지능 프로젝트 임에도 불구하고 EDA나 전처리 때의 라이브러리 활용, 알고리즘 등의 능력과 같이 기본적인 프로그래밍 역량이 요구되었습니다. 어떻게 보면 당연한 사실이긴 하지만, 늘 짜여진 템플릿에 의존하여 모델링에만 의존하던 모습을 보였던 것 같습니다. 컨벤션이나 템플릿을 갖추는 것은 물론, 효율적인 프로그램을 설계하고 제작하는 것이 모든 분야 개발자의 주요 역량이라는 점을 다시금 새겼습니다.

## 아이디어의 변경
본 프로젝트는 진행한 교내 *오픈소스 프로젝트* 강의를 통해 진행하였습니다.

강의 담당 교수님께서 기존 아이디어를 Document layout analysis 분야로 방향을 변경하였고, Document의 Layout같은 경우는 배경 정보가 없기 때문에 기존 실험과는 거리가 멀어졌습니다.
...
또한, Document Layout Analysis 분야에서 S.O.T.A.정확도가 약 93%로 기록되어 있습니다. 본 연구에서 비록 학습 mAP@0.5이긴 하지만 약 95%의 정확도를 도출한 것에서 의구심이 들었습니다. 카테고리를 더욱 쉽게 분류할 수 있도록 재구성하긴 하였지만, 논문 신청 일자와 과목 최종 프로젝트 등록 일자를 맞추다 보니 말도 안되게 높은 수치에 실험적 오류가 있었다고 생각이 들었습니다. 논문을 우습게 보고 신뢰성이 낮은 결과로 작성하여 죄책감을 크게 느낍니다. 이 점을 반성하여, 추후에는 보다 구체적은 계획을 바탕으로 신뢰성이 보장되는 결과를 제공하고자 합니다.