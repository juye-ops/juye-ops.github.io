...
|⚙ 개발 환경|💡오픈소스 & 라이브러리|
|:-:|:-:|
|`Docker` `MongoDB` `NginX` `Python` `React`|`FastAPI` `Docker-py` `Code-server`|
...
- `Docker-compose` 환경 구성
  ...
  - container: Backend와 연결된 SSL 기반 Docker in docker 및 `Code-server` IDE 제공
  - database: 클라이언트가 생성한 개발 컨테이너 정보 관리
- 웹 서버 Proxy 설정
